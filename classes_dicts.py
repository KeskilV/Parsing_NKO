...
class Dicts():
    ...
    def load(self, filename=None, dictname=None):
        ...
        if dictname == None:
            ...
            c = int(input('номер: '))
            dictname = self.dict[list(self.dict.keys())[c]]
            print (f'выбрано - {self.name}')
            self.name = dictname = self.dict[dictname]
